...
content = content.replace('<div className="absolute top-0 right-0 w-64 h-64 bg-green-50 dark:bg-green-500/10 dark:border-green-500/20 rounded-full blur-3xl -mr-20 -mt-20 opacity-50 pointer-events-none"></div>', '');

// 2. Change text
content = content.replace('"Aguardando primeira entrega para iniciar"', '"Aguardando Início"');
