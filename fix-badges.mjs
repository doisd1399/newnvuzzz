...
function fixDir(dir) {
  for (const f of fs.readdirSync(dir)) {
    ...
    if (fs.statSync(fn).isDirectory()) {
      ...
      const colors = ['blue', 'green', 'emerald', 'orange', 'red', 'rose', 'indigo', 'purple', 'amber', 'yellow'];
      
      for (const color of colors) {
         // match class names containing bg-{color}-50 and text-{color}-600 (or 700 or whatever)
         let regex = new RegExp(`className="([^"]*bg-${color}-50(?!0)[^"]*)"`, 'g');
         c = c.replace(regex, (match, classNames) => {
            if (!classNames.includes('dark:bg-') && classNames.includes(`text-${color}-`)) {
               return `className="${classNames.replace(`bg-${color}-50`, `bg-${color}-50 dark:bg-${color}-500/10`)}"`;
            }
            return match;
         });
         
         let textRegex = new RegExp(`className="([^"]*text-${color}-(600|700|800)[^"]*)"`, 'g');
         c = c.replace(textRegex, (match, classNames) => {
            if (!classNames.includes('dark:text-') && classNames.includes(`bg-${color}-50`)) {
