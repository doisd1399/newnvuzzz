...
code = code.replace(
  /const applicationRef = await addDoc\(\s*collection\(db, "recruitment_applications"\),\s*\{[^}]+\},\s*\);/g,
  `let applicationRef;
      try {
...
code = code.replace(
  /await setDoc\(\s*doc\(db, "users", auth\.currentUser\.uid\),\s*\{\s*applicationSubmitted: true,\s*status: "pending", \/\/ Update their own user status\s*\},\s*\{\s*merge: true\s*\},\s*\);/g,
  `try {
          await setDoc(
