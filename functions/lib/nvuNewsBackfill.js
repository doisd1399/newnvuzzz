...
exports.generateNvuNewsScheduled = exports.generateNvuNewsBackfill = void 0;
...
const DRIVER_EARNINGS_TEMPLATES = {
    dia: [
...
const COMPANY_EARNINGS_TEMPLATES = {
    dia: [
...
function zonedDate(year, month, day, hour = 0, minute = 0, second = 0, millisecond = 0) {
...
function incrementalRangeStart(lastGenerationKey, now) {
...
function formatWeekdayPt(date, short = false) {
...
function stableTemplateIndex(seed, length) {
    if (length <= 1)
        return 0;
    const hash = (0, node_crypto_1.createHash)("sha256").update(seed).digest("hex");
...
function companyNameOf(aggregate, company) {
    return firstNonEmpty(company === null || company === void 0 ? void 0 : company.companyName, company === null || company === void 0 ? void 0 : company.fleetName, company === null || company === void 0 ? void 0 : company.name, aggregate.companyName);
}
...
function driverNameOf(aggregate, user) {
    return shortPersonName(firstNonEmpty(user === null || user === void 0 ? void 0 : user.name, user === null || user === void 0 ? void 0 : user.fullName, user === null || user === void 0 ? void 0 : user.displayName, aggregate.driverName));
}
...
function addToAggregate(map, key, trip, entity, simulator) {
    ...
    if (!existing.driverName && trip.driverName)
        existing.driverName = trip.driverName;
    if (!existing.companyName && trip.companyName)
        existing.companyName = trip.companyName;
    map.set(key, existing);
}
function selectTemplates(entity, metric, period) {
    if (entity === "motorista" && metric === "viagens")
        return DRIVER_TRIP_TEMPLATES[period];
    if (entity === "motorista" && metric === "ganhos")
        return DRIVER_EARNINGS_TEMPLATES[period];
    if (entity === "empresa" && metric === "viagens")
        return COMPANY_TRIP_TEMPLATES[period];
...
function contentDigest(data) {
    ...
    return (0, node_crypto_1.createHash)("sha256").update(JSON.stringify(relevant)).digest("hex");
}
function buildNewsDocument(entity, metric, period, aggregate, company, user) {
    ...
    if (!empresaNome || (entity === "motorista" && !motoristaNome))
        return null;
    const dedupeKey = [
        AUTOMATION_VERSION,
    ...
    const data = {
        ...
        searchTokens: buildSearchTokens(motoristaNome, empresaNome, aggregate.simulator.name, aggregate.simulator.id, titleFor(entity, period.type, metric, period), mensagem, period.label, category),
    };
    const contentHash = contentDigest(data);
    data.contentHash = contentHash;
    return {
        documentId: `news_v6_${(0, node_crypto_1.createHash)("sha256").update(dedupeKey).digest("hex").slice(0, 40)}`,
        dedupeKey,
        contentHash,
        data,
    };
}
function buildPeriodNews(period, periodTrips, companies, users) {
    const tripsBySimulator = new Map();
    ...
    tripsBySimulator.forEach(({ simulator, trips }) => {
        ...
        if ((topDriverTrips === null || topDriverTrips === void 0 ? void 0 : topDriverTrips.trips) > 0) {
            append(buildNewsDocument("motorista", "viagens", period, topDriverTrips, companies.get(topDriverTrips.companyId), users.get(topDriverTrips.driverId)));
        }
        if (topDriverEarnings) {
            append(buildNewsDocument("motorista", "ganhos", period, topDriverEarnings, companies.get(topDriverEarnings.companyId), users.get(topDriverEarnings.driverId)));
        }
        if ((topCompanyTrips === null || topCompanyTrips === void 0 ? void 0 : topCompanyTrips.trips) > 0) {
            append(buildNewsDocument("empresa", "viagens", period, topCompanyTrips, companies.get(topCompanyTrips.companyId), undefined));
        }
        if (topCompanyEarnings) {
            append(buildNewsDocument("empresa", "ganhos", period, topCompanyEarnings, companies.get(topCompanyEarnings.companyId), undefined));
        }
    });
    return generated;
}
function parseRecordDescriptor(category) {
    ...
    return {
        entity: match[1],
        period: match[2],
        metric: match[3] === "movimentacao" ? "ganhos" : match[3],
    };
}
function recordCategory(descriptor) {
...
function recordTitle(descriptor) {
...
function recordMessage(descriptor, source) {
...
function buildHistoricalRecordCandidates(generated, now) {
    ...
    generated.forEach((news) => {
        ...
        if (!current || value > current.value)
            bestByRecord.set(key, { news, value, descriptor });
    });
    return Array.from(bestByRecord.entries()).map(([recordKey, candidate]) => {
        ...
        const data = Object.assign(Object.assign({}, source), { categoria: category, titulo: recordTitle(candidate.descriptor), mensagem: recordMessage(candidate.descriptor, source), tipo: "automatico", origem: "recorde_historico_automatico", recordeHistorico: true, recordeEntidade: candidate.descriptor.entity, recordeMetrica: candidate.descriptor.metric, recordePeriodo: candidate.descriptor.period, recordeValor: candidate.value, recordePeriodoOriginal: source.periodo, recordeAtingidoEm: source.publicacaoProgramadaEm, publicacaoProgramadaEm: admin.firestore.Timestamp.fromDate(now), dataCriacao: admin.firestore.Timestamp.fromDate(now), dedupeKey, automationVersion: AUTOMATION_VERSION, createdBySystem: true, searchTokens: buildSearchTokens(source.motoristaNome, source.empresaNome, source.simulador, category, recordTitle(candidate.descriptor), recordMessage(candidate.descriptor, source), source.periodo, "recorde histórico") });
        const contentHash = contentDigest(data);
        data.contentHash = contentHash;
        return {
            documentId: `news_record_v1_${(0, node_crypto_1.createHash)("sha256").update(dedupeKey).digest("hex").slice(0, 36)}`,
            dedupeKey,
            contentHash,
            data,
        };
...
async function commitHistoricalRecords(generated) {
    let created = 0;
    let updated = 0;
    let ignored = 0;
    for (let offset = 0; offset < generated.length; offset += 100) {
        ...
        chunk.forEach((item, index) => {
            ...
            if (!snapshot.exists) {
                batch.create(refs[index], Object.assign(Object.assign({}, item.data), { curtidasCount: 0, comentariosCount: 0, createdAt: admin.firestore.FieldValue.serverTimestamp(), lastCalculatedAt: admin.firestore.FieldValue.serverTimestamp() }));
                created += 1;
                hasWrites = true;
                return;
            }
            ...
            batch.set(refs[index], Object.assign(Object.assign({}, item.data), { curtidasCount: Math.max(0, Number(current.curtidasCount || 0)), comentariosCount: Math.max(0, Number(current.comentariosCount || 0)), previousRecordValue: currentValue, lastCalculatedAt: admin.firestore.FieldValue.serverTimestamp() }), { merge: true });
            updated += 1;
            hasWrites = true;
        });
        if (hasWrites)
...
function collectPeriods(trips, now, periodTypes, minimumPeriodStart) {
    ...
    trips.forEach((trip) => {
        periodTypes.forEach((periodType) => {
            ...
            if (!descriptor)
                return;
            if (minimumPeriodStart && descriptor.start.getTime() < minimumPeriodStart.getTime())
                return;
...
function generatedNewsFromPeriods(periods, companies, users) {
...
async function acquireAutomationLock(runId, generationKey) {
...
function isLegacyAutomaticNews(data) {
...
async function archiveLegacyAutomaticNews() {
...
async function commitGeneratedNews(generated) {
    let created = 0;
    let updated = 0;
    let ignored = 0;
    const chunkSize = 180;
    for (let offset = 0; offset < generated.length; offset += chunkSize) {
        ...
        chunk.forEach((item, index) => {
            ...
            batch.set(refs[index], Object.assign(Object.assign({}, item.data), { curtidasCount: Math.max(0, Number(current.curtidasCount || 0)), comentariosCount: Math.max(0, Number(current.comentariosCount || 0)), lastCalculatedAt: admin.firestore.FieldValue.serverTimestamp() }), { merge: true });
            updated += 1;
            hasWrites = true;
        });
        if (hasWrites)
...
async function generateCompletedNews(source) {
    const now = new Date();
    ...
    try {
        ...
        const recordSourceGenerated = historicalTrips.length > 0
            ? generatedNewsFromPeriods(collectPeriods(historicalTrips, now, ["semana", "mes"]), companies, users)
