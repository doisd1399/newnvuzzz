...
export type PeriodOption = 'today' | '7days' | '15days' | 'thisMonth' | 'custom';
...
export function PeriodSelector({ onPeriodChange }: PeriodSelectorProps) {
  ...
  while (day <= endDate) {
    for (let i = 0; i < 7; i++) {
      ...
      const isSelected = customRange.from && customRange.to 
        ? isWithinInterval(cloneDay, { start: customRange.from, end: customRange.to }) 
        : customRange.from && cloneDay.getTime() === customRange.from.getTime();
        
      const isStart = customRange.from && cloneDay.getTime() === customRange.from.getTime();
      const isEnd = customRange.to && cloneDay.getTime() === customRange.to.getTime();
      ...
      days.push(
        <div
          ...
          className={cn(
            ...
            isSelected && isCurrentMonth && !isStart && !isEnd ? "bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-none" : "",
            (isStart || isEnd) && isCurrentMonth ? "bg-blue-600 text-white hover:bg-blue-700 rounded-md font-semibold" : ""
          )}
        >
          {formattedDate}
        </div>
  ...
  const getButtonLabel = () => {
    if (activeOption !== 'custom') {
      return options.find(o => o.id === activeOption)?.label || 'Últimos 7 dias';
    }
    ...
    <div className="relative">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[14px] font-semibold text-slate-800 dark:text-slate-200">{getButtonLabel()}</span>
        </div>
        <ChevronDown size={14} className={cn("text-slate-400 shrink-0 transition-transform", isOpen && "rotate-180")} />
      </button>

      {isOpen && (
        <>
          <div className="fixed inset-0 z-[100] bg-black/20 sm:bg-transparent backdrop-blur-sm sm:backdrop-blur-none" onClick={() => setIsOpen(false)} />
              ...
              {options.map((opt) => (
                <button
                  ...
                  className={cn(
                    ...
                    activeOption === opt.id ? "bg-slate-50 dark:bg-[#2A2F3A]" : "hover:bg-slate-50 dark:hover:bg-[#2A2F3A]/50"
                  )}
                >
                  <span className={cn(
                    "text-[14px] font-medium",
                    activeOption === opt.id ? "text-slate-900 dark:text-white" : "text-slate-600 dark:text-slate-300"
                  )}>
                    {opt.label}
                  </span>
                  {activeOption === opt.id && <Check size={16} className="text-blue-600 dark:text-blue-400" />}
                </button>
              ))}
            </div>
              ...
              <div className="p-4 w-full sm:w-[280px]">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-1">
                    <button onClick={prevMonth} className="p-1.5 hover:bg-slate-100 dark:hover:bg-[#2A2F3A] rounded-lg text-slate-500 transition-colors">
                      <ChevronLeft size={16} />
                    </button>
                    <button onClick={nextMonth} className="p-1.5 hover:bg-slate-100 dark:hover:bg-[#2A2F3A] rounded-lg text-slate-500 transition-colors">
                ...
                <div className="grid grid-cols-7 mb-2">
                  {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((day, i) => (
                    <div key={i} className="text-center text-[12px] font-semibold text-slate-400 mb-2">
                      {day}
                    </div>
                  ))}
                  {days}
