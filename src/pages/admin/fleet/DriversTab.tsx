...
export default function DriversTab() {
  ...
  const activeEmployees = allEmployees.filter((u) =>
  ...
  const sortedEmployees = [...filteredEmployees]
    .map((driver) => {
      const { allCompletedJobs, activeJob, totalDeliveries, displayLevel, xpProgress } = getDriverLevelData(driver.id, jobs, contracts, historicoTrips);
      return { ...driver, totalDeliveries, allDriverJobs: allCompletedJobs, activeJob, displayLevel, xpProgress };
    })
                  ...
                  <CardContent className="p-0">
                    <div className="flex flex-col sm:flex-row">
                      ...
                      <div className="bg-white dark:bg-[#1A1F26] p-6 flex flex-col justify-center sm:border-l border-yellow-100 dark:border-yellow-500/20 sm:w-40 shrink-0 border-t sm:border-t-0 gap-3">
                        ...
                        <Button
                          onClick={() => navigate(`/admin/driver/${driver.id}`, { state: { returnUrl: "/admin/fleet", returnState: { activeTab: "drivers" } } })}
                          variant="ghost"
                               ...
                               <div className="absolute right-0 top-full mt-1 w-52 bg-white dark:bg-[#1A1F26] border border-gray-200 dark:border-gray-800 rounded-xl shadow-lg py-1.5 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                                  <button 
                                    onClick={() => {
                                      ...
                                      navigate(`/admin/driver/${driver.id}`, { state: { returnUrl: "/admin/fleet", returnState: { activeTab: "drivers" } } });
                                    }}
