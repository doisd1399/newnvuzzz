...
export default function VehiclesTab() {
  const { vehicles, addVehicle, updateVehicle, deleteVehicle, jobs } = useAppStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      updateVehicle(editingId, { name, paintCode });
    } else {
      addVehicle({ name, paintCode });
    }
    resetForm();
  };
  ...
  const getUsageCount = (vehicleId: string) => {
    // Conta quantos motoristas únicos estão usando este veículo em trabalhos pendentes ou ativos
    const activeJobs = jobs.filter(j => j.vehicleId === vehicleId && ['pending', 'active'].includes(j.status));
    ...
    <div className="space-y-6">
        ...
        <Card className="rounded-2xl border border-gray-200 dark:border-[#2A2F3A] shadow-sm dark:shadow-none mb-6 bg-white dark:bg-[#1A1F26] overflow-hidden relative">
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-center mb-6">
               <h2 className="text-lg font-semibold text-gray-900 dark:text-[#fafafa]">Novo Veículo</h2>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 dark:text-[#d4d4d8] mb-1">Modelo do Caminhão</label>
                <input 
                  type="text" 
                  value={name} 
              ...
              <div className="md:col-span-2">
                ...
                <input 
                  ...
                  placeholder="Ex: #FF0000 ou Vermelho Metálico"
                  className="w-full bg-gray-50 dark:bg-[#1A1F26] border border-gray-200 dark:border-[#2A2F3A] rounded-xl px-4 py-3 text-gray-900 dark:text-[#fafafa] focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-white dark:focus:bg-[#0a0a0b] transition-colors"
                />
              </div>
              <div className="md:col-span-2 flex justify-end gap-3 mt-4">
                <Button type="button" variant="ghost" onClick={resetForm} className="text-gray-600 dark:text-[#d4d4d8]">Cancelar</Button>
                <Button type="submit" className="bg-green-600 dark:bg-green-600 hover:bg-green-700 text-white shadow-sm dark:shadow-none border-none">{editingId ? 'Salvar Alterações' : 'Salvar Veículo'}</Button>
              </div>
            </form>
          </CardContent>
        </Card>
      ...
      {vehicles.length === 0 ? (
         <div className="text-center py-20 bg-white dark:bg-[#1A1F26] rounded-2xl border border-gray-200 dark:border-[#2A2F3A] border-dashed">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-[#fafafa]">Nenhum veículo encontrado</h3>
            <p className="text-gray-500 dark:text-[#a1a1aa] mt-1 max-w-sm mx-auto">Adicione um novo veículo para sua frota usando o botão acima.</p>
         </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 lg:gap-6">
          {vehicles.map(vehicle => {
            const usageCount = getUsageCount(vehicle.id);
            return (
              <Card key={vehicle.id} className="rounded-xl border border-gray-200 dark:border-[#2A2F3A] shadow-none relative overflow-hidden group hover:shadow-sm dark:shadow-none transition-shadow dark:shadow-none bg-white dark:bg-[#1A1F26]">
                <CardContent className="p-3 md:p-4">
                   <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-semibold text-gray-900 dark:text-[#fafafa] text-[14px] leading-tight mb-1">{vehicle.name}</h3>
                      ...
                      <div className="flex gap-1">
                        <button onClick={() => handleEdit(vehicle.id, vehicle.name, vehicle.paintCode)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:bg-blue-500/10 dark:border-blue-500/20 rounded-lg transition-colors" title="Editar"><Pencil size={16} /></button>
                        <button onClick={() => handleDelete(vehicle.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:bg-red-500/10 dark:border-red-500/20 rounded-lg transition-colors" title="Excluir"><Trash2 size={16} /></button>
                      </div>
                   </div>
                </CardContent>
              </Card>
