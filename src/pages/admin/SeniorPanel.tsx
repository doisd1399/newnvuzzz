...
export default function SeniorPanel() {
  ...
  const [activeTab, setActiveTab] = useState<
    "requests" | "approved" | "profile" | "settings"
  >("requests");
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      unsubs.push(
        onSnapshot(collection(db, "frotas"), (snap) =>
          setAllCompanies(snap.docs.map((d) => ({ id: d.id, ...d.data() }))),
        ),
      );
      unsubs.push(
        onSnapshot(collection(db, "companyMembers"), (snap) =>
      ...
      unsubs.push(
        onSnapshot(collection(db, "reboques"), (snap) =>
  ...
  const handleReject = async (id: string) => {
    try {
      ...
      // Let's create a notification (optional but we can try)
      ...
      if (reg && reg.userId) {
        ...
        batch.set(notifRef, {
          ...
          message: "Sua solicitação de empresa foi recusada. Consulte os detalhes enviados pela administração.",
          lida: false,
          tipo: "admin",
          dataHora: new Date().toISOString()
        });
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded-xl border border-slate-200/60 dark:border-slate-800 w-full mb-1">
                          <div className="relative shrink-0">
                            {reg.photoURL ? (
                              <button onClick={() => setZoomedImage(reg.photoURL!)} className="block w-12 h-12 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm relative hover:opacity-80 transition-opacity">
                                <img
                                  src={reg.photoURL}
                                  alt="Logo"
                                  className="w-full h-full object-cover"
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-slate-500 dark:text-[#a1a1aa] truncate">
                              {reg.photoURL ? "Clique para ampliar" : "Não enviada"}
                            </p>
                          </div>
                        </div>
