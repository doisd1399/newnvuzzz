...
export default function RegisterCompany() {
  ...
  useEffect(() => {
    if (currentUser) {
      setFormData(prev => ({
        ...
        ownerPhotoUrl: prev.ownerPhotoUrl || currentUser.photoURL || currentUser.avatar || ''
      }));
  ...
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>, isOwnerPhoto: boolean = false) => {
    ...
    try {
      ...
      if (ownerPhotoPreview && ownerPhotoPreview.startsWith('data:image')) {
        ...
        try {
          ...
          console.warn("Falha no upload da foto do proprietário. Usando foto padrão.", uploadError);
      ...
      await addDoc(collection(db, 'recruitment_applications'), {
        ...
        ownerPhotoUrl: uploadedOwnerPhotoUrl || currentUser?.photoURL || currentUser?.avatar || '',
        status: 'pending',
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#09090b] flex flex-col items-center p-4 sm:p-6 overflow-y-auto">
      <div className="w-full max-w-[440px] my-auto py-8">
         <div className="relative mb-6">
            {!isSubmitted && (
              <button type="button" onClick={handleCancel} className="absolute left-0 top-1/2 -translate-y-1/2 p-2 w-10 h-10 text-slate-500 hover:text-slate-900 dark:text-[#a1a1aa] dark:hover:text-white transition-colors bg-white dark:bg-[#1A1F26] rounded-full border border-slate-200 dark:border-[#2A2F3A] shadow-sm flex items-center justify-center group" title="Voltar para a tela anterior">
                 <ArrowLeft size={18} className="group-hover:-translate-x-0.5 transition-transform" />
              </button>
            )}
            <div className="text-center">
               <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-[#fafafa] tracking-tight mb-1">NVU</h1>
               <p className="text-slate-500 dark:text-[#a1a1aa] text-sm font-medium">Cadastro de Nova Empresa</p>
            </div>
         </div>
            ...
            <CardContent className="p-5 sm:p-7">
              {isSubmitted ? (
                <div className="flex flex-col items-center text-center py-8">
                   ...
                   <h2 className="text-[20px] font-bold text-slate-900 dark:text-[#fafafa] mb-3 tracking-tight">Cadastro Enviado</h2>
                   <p className="text-slate-500 dark:text-[#a1a1aa] mb-8 text-[15px] leading-relaxed">
                      Sua solicitação de cadastro foi enviada com sucesso e está aguardando aprovação.<br/><br/>
                      Você receberá um contato assim que sua empresa for aprovada.
                   </p>
                   <Button onClick={handleCancel} variant="outline" className="w-full h-12 rounded-xl text-[15px] font-semibold text-slate-600 dark:text-[#f4f4f5] hover:text-slate-900 dark:hover:text-[#fafafa]">
                      Voltar para o Login
                   </Button>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="relative group shrink-0">
                      {photoPreview ? (
                        ...
                        <label className="w-14 h-14 rounded-full bg-slate-100 dark:bg-[#2A2F3A] border border-dashed border-slate-300 dark:border-[#3f3f46] flex flex-col items-center justify-center cursor-pointer hover:bg-slate-200 dark:hover:bg-[#3f3f46] transition-colors">
                          <ImageIcon size={18} className="text-slate-400 dark:text-[#71717a]" />
                          <input type="file" accept="image/*" className="hidden" onChange={(e) => handleImageSelect(e, false)} />
                        </label>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                       <label className="block text-[14px] font-semibold text-slate-800 dark:text-[#d4d4d8] mb-0.5">Logo da Empresa</label>
                       {photoPreview ? (
                         <button type="button" onClick={() => { setPhotoPreview(null); setFormData(f => ({...f, photoURL: ''})); }} className="text-rose-500 hover:text-rose-600 text-[12px] font-medium flex items-center gap-1 transition-colors">
                           Remover foto
                         </button>
                       ) : (
                         <p className="text-[12px] text-slate-500 dark:text-[#a1a1aa] truncate">Envie uma imagem opcional</p>
                       )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 bg-slate-50/50 dark:bg-[#18181b] p-3.5 rounded-2xl border border-slate-200 dark:border-[#2A2F3A]/60 w-full mb-1">
                    <div className="relative group shrink-0">
                      {ownerPhotoPreview ? (
                        <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-white dark:border-[#2A2F3A] shadow-sm relative">
                          <img src={ownerPhotoPreview} alt="Owner Preview" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <label className="cursor-pointer text-white p-2">
                              <Upload size={16} />
                    ...
                    <div className="flex-1 min-w-0">
                       <label className="block text-[14px] font-semibold text-slate-800 dark:text-[#d4d4d8] mb-0.5">Foto do Proprietário</label>
                       {ownerPhotoPreview ? (
                         <button type="button" onClick={() => { setOwnerPhotoPreview(null); setFormData(f => ({...f, ownerPhotoUrl: ''})); }} className="text-rose-500 hover:text-rose-600 text-[12px] font-medium flex items-center gap-1 transition-colors">
                           Remover foto
                         </button>
                       ) : (
                         <p className="text-[12px] text-slate-500 dark:text-[#a1a1aa] truncate">Entrará no seu perfil de motorista</p>
                       )}
                    </div>
                  </div>

                  <div>
                    <label className="block text-[13px] font-medium text-slate-700 dark:text-[#d4d4d8] mb-1.5 ml-0.5">Nome da Empresa</label>
                    <input 
                      required
                  ...
                  <div>
                    <label className="block text-[13px] font-medium text-slate-700 dark:text-[#d4d4d8] mb-1.5 ml-0.5">Nome da Frota</label>
                    <input 
                      required
                  ...
                  <div>
                    <label className="block text-[13px] font-medium text-slate-700 dark:text-[#d4d4d8] mb-1.5 ml-0.5">Nome do Proprietário</label>
                    <input 
                      required
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[13px] font-medium text-slate-700 dark:text-[#d4d4d8] mb-1.5 ml-0.5">Email</label>
                      <input 
                        required type="email"
                        inputMode="email"
                    ...
                    <div>
                      <label className="block text-[13px] font-medium text-slate-700 dark:text-[#d4d4d8] mb-1.5 ml-0.5">WhatsApp</label>
                      <input 
                        required type="tel"
                        inputMode="tel"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[13px] font-medium text-slate-700 dark:text-[#d4d4d8] mb-1.5 ml-0.5">CNPJ <span className="font-normal opacity-70">(Automático)</span></label>
                      <input 
                        required
                        disabled
                        value={formData.cnpj}
                    ...
                    <div>
                      <label className="block text-[13px] font-medium text-slate-700 dark:text-[#d4d4d8] mb-1.5 ml-0.5">Simulador</label>
                      <select 
                        required
                  ...
                  <div className="pt-2 flex flex-col sm:flex-row gap-3">
                    <Button type="button" onClick={handleCancel} variant="outline" className="w-full sm:w-1/3 rounded-xl h-12 text-[15px] font-semibold border-slate-200 dark:border-[#2A2F3A]">Cancelar</Button>
                    <Button disabled={submitting} type="submit" className="w-full sm:w-2/3 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white rounded-xl h-12 text-[15px] font-bold shadow-sm transition-all">
