...
export const CompanyPerformanceCard = ({
  historicoTrips,
  companyId,
  allCompanies,
  ...
  const currentTrips = companyTrips.filter((t) => {
    return t.metricDate >= sDate && t.metricDate <= eDate;
  });

  const prevTrips = companyTrips.filter((t) => {
    ...
    for (let i = 0; i < numPeriods; i++) {
      ...
      normalizedHistorico.forEach((trip) => {
        if (trip.metricDate >= periodStart && trip.metricDate <= periodEnd) {
          ...
          companyStatsMap[mId].val += trip.normalizedValor;
        }
      });
