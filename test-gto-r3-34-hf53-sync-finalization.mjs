...
ck('HF53 Android identity', gradle.includes('versionCode 105') && gradle.includes('versionName "1.0.105"'));
ck('HF53 workflow identity', workflow.includes('PC-HF53') && workflow.includes('EXPECTED_VERSION_CODE: "105"') && workflow.includes('EXPECTED_VERSION_NAME: "1.0.105"'));
...
ck('print receipt has explicit preparation/validation/upload phases', recordTrip.includes('type ReceiptUploadPhase = "idle" | "preparing" | "validating" | "uploading";') && recordTrip.includes('setUploadPhase("validating")') && recordTrip.includes('setUploadPhase("uploading")'));
ck('receipt overlay no longer uses generic isUploading state', recordTrip.includes('{uploadPhase !== "idle" && (') && !recordTrip.includes('{isUploading && (\n                    <div className="absolute inset-x-0 bottom-0'));
ck('0 percent is shown only during real Storage upload phase', recordTrip.includes('{uploadPhase === "uploading" && <span>{uploadProgress.toFixed(0)}%</span>}'));
...
ck('final trip submission does not masquerade as receipt upload', !submit.includes('setUploadPhase("uploading")'));
...
ck('registerGtoTrip keeps destinationCompany optional', recordTrip.length > 0 && read('functions/src/gtoTrips.ts').includes('if (destinationCompany) assertBoundedText(destinationCompany, "destinationCompany");'));
ck('HF52 native sync hardening preserved byte-for-byte', sha('android/app/src/main/java/com/nvu/operacional/GtoAutoTripSync.java') === 'f8d0065098f7ea1981396e73862c0a4507c71a7ce9249d540b82bfd688a81910' && sha('android/app/src/main/java/com/nvu/operacional/GtoObserverService.java') === '6e2bd629f2186693dd95f6bd6545f2b1c6187b3fc030a15c73bf02a262226f66' && sha('android/app/src/main/java/com/nvu/operacional/GtoObserverPlugin.java') === '168ecd219ca96528327fbcb615dcd0d591415d393db8a1e930f5268cf2d8d7a1');
...
console.log(`\n${checks.length - failed.length}/${checks.length} HF53 sync/finalization checks passed.`);
