...
function ensureTeardownListener() {
  ...
  onAuthTeardown(() => {
    ...
    retryTimer = null;
    retryAttempt = 0;
    cachedState = { trips: [], loading: false };
    hasLoadedOnce = false;
    notifySubscribers();
  });
}
...
function ensureSubscription() {
  ...
  unsubscribe = TripsRepository.listenAllTrips(
    ...
    (error) => {
      if (isAuthTeardownActive()) return;
      console.warn("Error fetching trips:", error);
      ...
      if (subscribers.size > 0 && !retryTimer) {
        ...
        retryTimer = setTimeout(() => {
          retryTimer = null;
          if (subscribers.size > 0 && !isAuthTeardownActive()) {
            ensureSubscription();
          }
        }, delay);
      }
...
export function useTripsRealtime() {
  ...
  useEffect(() => {
    ...
    if (!isAuthTeardownActive()) ensureSubscription();
    updateState();
