...
def update_file(filepath, is_driver):
    ...
    xp_label = "{Math.floor(currentLevelXp)} / 1.000 XP" if is_driver else "800 / 1.000 XP"
    xp_percent = "{Math.round(xpProgress)}%" if is_driver else "80%"
    xp_width = "`${Math.max(2, xpProgress)}%`" if is_driver else "'80%'"
    level_label = "{currentLevel}" if is_driver else "12"
            ...
            <div className="flex flex-row items-center justify-between gap-1 sm:gap-10 mt-0 sm:mt-1">
              <div className="flex items-center gap-1.5 sm:gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] sm:text-[18px] font-bold text-white leading-none">92%</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 sm:gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] sm:text-[18px] font-bold text-white leading-none">89%</span>
                </div>
              </div>
              <div className="w-px h-5 sm:h-8 bg-white/10 shrink-0"></div>
              <div className="flex items-center gap-1.5 sm:gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] sm:text-[18px] font-bold text-white leading-none">91%</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-row items-center gap-2 sm:gap-4 mt-1 sm:mt-2 bg-[#021317] rounded-[10px] sm:rounded-2xl p-1.5 sm:p-3 border border-white/5">
              <div className="flex-1 flex flex-col gap-1 sm:gap-2">
                <div className="flex justify-between items-center px-1">
                   <span className="text-[9px] sm:text-[14px] font-medium text-white/90">{xp_label}</span>
                   <span className={{cn("text-[9px] sm:text-[13px] font-semibold", bannerTextColor)}}>{xp_percent}</span>
                </div>
                <div className="w-full h-1.5 sm:h-2.5 bg-[#0a2e38] rounded-full overflow-hidden">
                  <div className={{cn("h-full rounded-full", bannerBgColor)}} style={{{{"width": {xp_width}}}}}></div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 pr-0.5 sm:pr-2">
                 ...
                 <span className="text-[9px] sm:text-[14px] font-bold text-white uppercase tracking-wider">
                   Nível <span className={{cn("text-[11px] sm:text-[16px] ml-0.5", bannerTextColor)}}>{level_label}</span>
                 </span>
              </div>
            </div>
          </div>
...
update_file("src/components/CompanyPerformanceCard.tsx", False)
update_file("src/components/DriverPerformanceCard.tsx", True)
