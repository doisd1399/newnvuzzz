...
export default function Login() {
  const { setCurrentUser, currentUser, authInitialized, companies } =
    useAppStore();
    ...
    const checkAccess = async () => {
      try {
        if (authInitialized && currentUser?.id) {
          ...
          // 2. No active membership, check pending recruitment applications
  ...
  const unifyUserDocument = async (user: any) => {
    ...
    const currentRoles =
      targetDocData?.roles ||
      (finalRole === "admin" ? ["admin", "driver"] : ["driver"]);
    const finalStatus =
      finalRole === "admin" ? "active" : targetDocData?.status || "pending";
    let finalMemberships = targetDocData?.memberships || {};

    if (
      Object.keys(finalMemberships).length === 0 &&
      targetDocData?.companyId
    ) {
    ...
    for (const oldId of docsToDelete) {
      ...
      const trabalhosQ = query(
        collection(db, "trabalhos"),
        where("motoristaId", "==", oldId),
      );
      const trabalhosQs = await getDocs(trabalhosQ);
      trabalhosQs.forEach((docSnap) =>
      ...
      const driverTrabalhosQ = query(
        collection(db, "trabalhos"),
        where("driverId", "==", oldId),
      );
      const driverTrabalhosQs = await getDocs(driverTrabalhosQ);
      driverTrabalhosQs.forEach((docSnap) =>
        batch.update(docSnap.ref, {
          motoristaId: user.uid,
          driverId: user.uid,
        }),
