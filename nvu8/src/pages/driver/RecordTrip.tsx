  ...
  const handleLancarViagem = async () => {
    ...
    try {
      ...
      if (activeJob && activeContract) {
        ...
        // Mantém progress sincronizado com a fonte real de viagens.
        ...
        const realProgress = tripsSnapshot.size;
        calculatedProgress = realProgress;
        const updates: any = { progress: realProgress };

        if (activeJob.status === "pending") {
          updates.status = "active";
        }

        if (realProgress >= activeContract.totalDeliveries) {
