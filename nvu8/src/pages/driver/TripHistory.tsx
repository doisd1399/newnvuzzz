...
import { collection, query, where, orderBy, limit, startAfter, getDocs, doc, updateDoc, deleteDoc, getAggregateFromServer, runTransaction, sum, count } from "firebase/firestore";
...
export default function TripHistory({
  ...
  const queryContextRef = React.useRef<string | null>(null);
  ...
  const loadTrips = async (isLoadMore = false) => {
    ...
    if (isLoadMore) {
      ...
      setLoading(true);
      const queryContext = `${activeCompanyId || ""}:${embeddedJob?.id || ""}`;
      const contextChanged = queryContextRef.current !== queryContext;

      // Keep the current list visible while only a filter changes. Clear it
      // when switching company/job so data from the previous context cannot
    ...
    try {
      ...
      if (deletingTrip.jobId && deletingTrip.contratoId) {
        await runTransaction(db, async (transaction) => {
          ...
          const jobRef = doc(db, "trabalhos", deletingTrip.jobId as string);
          const contractRef = doc(db, "contratos", deletingTrip.contratoId as string);
          ...
          if (jobSnap.exists() && contractSnap.exists()) {
            ...
            const currentProgress = typeof jobData.progress === "number" ? jobData.progress : 0;
            const newProgress = Math.max(0, currentProgress - 1);
            const totalDeliveries = typeof contractData.totalDeliveries === "number" ? contractData.totalDeliveries : 0;

            console.log(`[DIAGNOSTIC] Dentro da transaction:`);
            console.log(`- documento de trabalho encontrado: ${jobSnap.id}`);
            console.log(`- progress antes: ${currentProgress}`);
            console.log(`- progress depois: ${newProgress}`);

            const updates: any = { progress: newProgress };

            if (jobData.status === "awaiting_completion" && newProgress < totalDeliveries) {
              updates.status = "active";
            }

            transaction.update(jobRef, updates);
          } else {
             console.log(`[DIAGNOSTIC] Transaction missing docs - jobSnap: ${jobSnap.exists()}, contractSnap: ${contractSnap.exists()}`);
          }
        });
      } else {
