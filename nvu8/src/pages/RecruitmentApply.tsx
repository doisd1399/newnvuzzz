...
export default function RecruitmentApply() {
  ...
  const userApplicationStatus = React.useMemo(() => {
    if (!currentUser || !company || !selectedSimulator) return 'eligible';
    ...
    const existingApps = recruitmentApplications.filter(a => 
      ...
      a.simulatorId === selectedSimulator
    );
    ...
    if (existingApps.length > 0) {
      ...
      const rejectedOrCanceled = existingApps.find(a => a.status === "rejected" || (a.status as any) === "cancelled" || (a.status as any) === "recusado" || (a.status as any) === "cancelado");
    ...
    return 'eligible';
  }, [currentUser, company, selectedSimulator, memberships, recruitmentApplications]);
  ...
  const availableSimulators = Array.from(
    new Set(
      allCompanies
        .filter((c) => c.recruitmentSettings?.isActive !== false)
        .map((c) => c.simulatorName)
        .filter(Boolean)
    ),
  ).sort();
  const filteredCompanies = allCompanies
    .filter((c) => c.simulatorName === selectedSimulator && c.recruitmentSettings?.isActive !== false)
  ...
  useEffect(() => {
    if (selectedCompanyId) {
      const c = allCompanies.find((c) => c.id === selectedCompanyId);
      setCompany(c || null);
    } else {
      setCompany(null);
    }
  ...
  const handleNext = async () => {
    ...
    if (currentUser) {
      ...
      setFormData((prev) => ({
        ...
        photoURL:
          prev.photoURL || currentUser.photoURL || currentUser.avatar || "",
      }));
    ...
    try {
      ...
      setPhotoPreview(resultUser.photoURL || null);
      setStep(2);
    } catch (err: any) {
