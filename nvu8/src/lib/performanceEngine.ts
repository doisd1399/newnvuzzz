...
function tripDate(trip: any): Date {
  if (trip?.metricDate instanceof Date) return trip.metricDate;
  if (trip?.metricDate?.toDate) return trip.metricDate.toDate();
  const raw = trip?.completedAt?.toDate?.() || trip?.completedAt || trip?.dataFechamento || trip?.date || trip?.dataLancamento || trip?.createdAt;
...
function tripValue(trip: any): number {
  const value = trip?.normalizedValor ?? trip?.valor ?? 0;
...
export function buildDriverRankingContext(
  ...
  const periodTrips = trips.filter((trip) => {
    if (trip?.isValid === false) return false;
