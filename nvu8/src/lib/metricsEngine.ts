...
function normalizeSimulatorName(value: unknown) {
  return String(value || "").trim().toLocaleLowerCase();
}

export function getTripSimulatorName(trip: NormalizedTrip | any, companies: any[] = []) {
  const explicitSimulator =
    trip?.simuladorNome || trip?.simulatorName || trip?.simulator;
  if (explicitSimulator) return String(explicitSimulator);

  const tripCompanyId = trip?.empresaId || trip?.companyId || trip?.company_id;
  if (!tripCompanyId) return undefined;

  return companies.find((company) => company.id === tripCompanyId)?.simulatorName;
}
...
export function filterTripsBySimulator(
  ...
  const targetSimulator = normalizeSimulatorName(simulator);
  if (
    !targetSimulator ||
    targetSimulator === "todos os simuladores" ||
    targetSimulator === "all"
  ) {
    return trips;
  }
  ...
  return trips.filter(
    (trip) =>
      normalizeSimulatorName(getTripSimulatorName(trip, companies)) ===
      targetSimulator,
  );
}
...
export function groupMetricsByCompany(
  ...
  const currentCompanies = (companies || []).filter(
    (company) =>
      !simulator ||
      simulator === "Todos os simuladores" ||
      simulator === "all" ||
      company.simulatorName === simulator,
  );
